    ...
    return (
        <div className='panel'>
            ...
            <div className="right-panel" >
                <div className='device1'><SmartTVCard /></div>
                <div className='device2'><LightCard /></div>
                <div className='device3'><AC /></div>
                <div className='device4'><SpeakerCard /></div>
                <div className='device5'><WifiRouterCard/></div>
                {devices.map((device, i) => (
                    <div key={i} className="added-device">{device}</div>
                ))}
                <button className="add-device" onClick={() => setShowModal(true)}>{t('add_device')}</button>
            </div>
            {showModal && (
